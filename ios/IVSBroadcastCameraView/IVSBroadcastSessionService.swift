...
enum BuiltInCameraUrns: String {
  ...
  case backTelephotoCamera = "camera:com.apple.avfoundation.avcapturedevice.built-in_video:2"
...
class IVSBroadcastSessionService: NSObject {
  ...
  public func setZoom(_ zoom: CGFloat) {
    let activeUrn = self.attachedCamera?.descriptor().urn
    let telephotoDevice = IVSBroadcastSession.listAvailableDevices().first { $0.urn == BuiltInCameraUrns.backTelephotoCamera.rawValue }
    let hasTelephotoDevice = telephotoDevice != nil
    ...
    if #available(iOS 13.0, *), zoom >= 0.5, zoom < 1 {
      if activeUrn != BuiltInCameraUrns.backUltraWideCamera.rawValue {
    ...
    } else if zoom >= 1, zoom < 2 || !hasTelephotoDevice {
      if activeUrn != BuiltInCameraUrns.backCamera.rawValue {
        self.attachCamera(urn: BuiltInCameraUrns.backCamera)
      }
    ...
    } else if zoom >= 2, hasTelephotoDevice {
      if activeUrn != BuiltInCameraUrns.backTelephotoCamera.rawValue {
        self.attachCamera(urn: BuiltInCameraUrns.backTelephotoCamera)
      }
